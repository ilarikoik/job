    ...
    <div className="bg-slate-700 min-h-screen p-5">
      ...
      <div className="h-fit w-full flex flex-col text-white mt-5 flex justify-center items-center ">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="bg-white text-black p-3 rounded-lg border-none hover:shadow-lg hover:shadow-black"
          >
            Lisää
          </button>
        </form>
      ...
      <div className="text-white p-3 flex flex-col justify-center items-center">
        <button
          onClick={() => setShowApply((prevApply) => !prevApply)}
          className={`font-bold text-2xl ${showApply && "text-red-300"}`}
        >
          {" "}
          {showApply
            ? "Piilota lista"
            : `Näytä tallennetut linkit: ${applyList.length}`}
        </button>
        {showApply &&
